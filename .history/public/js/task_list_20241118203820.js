...
// Function to display each task card
function displayMytaskCard(doc) {
    ...
    let taskId = doc.id;  // Task ID (Firestore document ID)
    ...
    // Create the delete button with the task ID
    let deleteButton = `<button type="button" class="btn btn-danger mt-auto mb-auto" data-bs-toggle="modal" data-bs-target="#deleteModal" onclick="setTaskToDelete('${taskId}')">Delete Task</button>`;

    // Clone and populate the task card template
    ...
    newcard.querySelector('.card-due').innerHTML = due.toLocaleDateString();
    newcard.querySelector('.accordion-body').innerHTML += deleteButton;
    ...
    let pillBadgeColor;
    let daysUntilDue = Math.floor((due - today) / (1000 * 60 * 60 * 24));
    ...
    // Update the task card with the formatted due date text
...
function deleteTask() {
    if (taskToDelete) {
        ...
        firebase.auth().onAuthStateChanged(user => {
            if (user) {
                // Reference to the task to delete
                const taskRef = db.collection("users").doc(user.uid).collection("tasks").doc(taskToDelete);
                
                // Delete the task from Firestore
                taskRef.delete().then(() => {
                    console.log("Task successfully deleted!");
                    // After deletion, close the modal and reload the tasks
                    var deleteModalEl = document.getElementById('deleteModal');
                    var deleteModal = bootstrap.Modal.getInstance(deleteModalEl);
                    deleteModal.hide();
                    getTasks();  // Reload the tasks
                }).catch(error => {
