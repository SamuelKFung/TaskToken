...
function displayMytaskCard(doc) { 
    ...
    let count = new Date().getTime(); // unique ID for collapse section
    let collapseID = "collapse" + count; 
    
    let pillBadgeColor = daysUntilDue < 0 ? "text-bg-danger" : "bg-success"; 
    ...
    let dueText = daysUntilDue == 0 ? "Due today!" : (daysUntilDue > 0 ? daysUntilDue + " days out" : Math.abs(daysUntilDue) + " days late");
    ...
    let deleteButton = newcard.querySelector('.btn-danger'); // Assuming there's a button with class 'btn-danger' in your template
    deleteButton.addEventListener('click', function() {
        deleteTask(doc.id); // Calling deleteTask function with the document ID as argument
    });
