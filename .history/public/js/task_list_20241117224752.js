...
// Assign a unique ID to each task card using the Firestore task ID
newcard.querySelector('.accordion').id = `task-${taskId}`;  // Add unique ID based on taskId
...
function deleteTask(taskId) {
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            // Reference to the specific task to delete
            ...
            taskRef.delete().then(() => {
                ...
                // Remove the task card from the UI (you can identify the task by the taskId)
                const taskCard = document.querySelector(`#task-${taskId}`);
                if (taskCard) {
                    taskCard.remove(); // Remove the task card element from the DOM
                }
