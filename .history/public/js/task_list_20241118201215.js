...
function displayMytaskCard(doc) { 
    ...
    let pillBadgeElement = name + "<span class=\"badge rounded-pill card-due fs-5 mx-4 mt-auto mb-auto " + pillBadgeColor + "\">" + daysUntilDue + "</span>"; 
    ...
    if (Math.abs(yearsUntilDue) < 1) { 
        if (Math.abs(monthsUntilDue) < 1) { 
            if (Math.abs(daysUntilDue) < 30) {
                // Prioritize days if less than 30
                if (daysUntilDue > 0) { 
                    dueText = daysUntilDue + " days out"; 
                    if (daysUntilDue == 1) {
                        dueText = "1 day out";
                    }
                } else if (daysUntilDue < 0) { 
                    dueText = -daysUntilDue + " days late"; 
                    if (daysUntilDue == -1) {
                        dueText = "1 day late";
                    }
                } else { 
                ...
                // Prioritize months if it's more than 30 days out
                if (monthsUntilDue > 0 && daysUntilDue >= 30) {
                    dueText = monthsUntilDue + " months out"; 
                    if (monthsUntilDue == 1 && daysUntilDue >= 30) {
                        dueText = "1 month out";
                    }
                } else {
                    dueText = -monthsUntilDue + " months late";
                    if (monthsUntilDue == -1) {
                        dueText = "1 month late";
                    }
                }
            }
        } else { 
        ...
        if (yearsUntilDue > 0) {
            dueText = yearsUntilDue + " years out"; 
            if (yearsUntilDue == 1) {
                dueText = "1 year out";
            }
        } else {
            dueText = -yearsUntilDue + " years late"; 
            if (yearsUntilDue == -1) {
                dueText = "1 year late";
            }
        }
