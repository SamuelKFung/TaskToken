document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addButton) {
        addButton.addEventListener('click', function() {
            ...
            // Your logic to show the modal or add the task
        });
    }
});
...
function displayMytaskCard(doc) {
    ...
    // Add add button event listener
    let addButton = newcard.querySelector('.btn-success'); // Assuming you have an add button in your template
    addButton.addEventListener('click', function () {
        addTask(doc); // Pass the entire document snapshot to the addTask function
    });
...
function addTask() {
    ...
    modalTitle.value = "";
    modalCourse.value = "";
    modalCategory.value = "Assignment"; // Reset to default category if necessary
    modalDate.value = "";
    modalDescription.value = "";

    // Change the modal title to "Add Task"
    const modalTitleContent = exampleModal.querySelector('.modal-title');
    modalTitleContent.textContent = "Add Task";

    // Remove the task ID attribute since we're adding a new task (not editing)
    exampleModal.removeAttribute('data-task-id');
}
