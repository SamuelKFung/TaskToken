...
function writeTasks(event) {
    ...
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            // Check if the taskId exists (if it's undefined, we're adding a new task)
            const taskId = document.getElementById('exampleModal').getAttribute('data-task-id');

            if (taskId) {
                // If taskId exists, we're editing an existing task
                ...
                // If taskId does not exist, we're adding a new task
                tasksRef.add({
                    name: taskName,
                    course: taskCourse,
                    category: taskCategory,
