...
// Sample function that generates task cards from Firestore data
function generateTaskCard(task) {
    const template = document.getElementById('taskCardTemplate');
    const taskCard = template.content.cloneNode(true);
    
    // Assuming task.id is the Firestore document ID
    const taskId = task.id; 

    // Set the task ID on the card itself (so we can find it later for deletion)
    taskCard.querySelector('.accordion-item').id = taskId;

    // Set the task name/title
    taskCard.querySelector('.card-name').textContent = task.title;

    // Set up the delete button to use the task ID
