...
function displayMytaskCard(doc) {
    ...
    // Check if the event is due within the same year
    if (Math.abs(yearsUntilDue) < 1) {
        if (Math.abs(monthsUntilDue) < 1) {
            // For days, handle both positive, negative, and "current day" cases
            if (daysUntilDue > 0) {
                dueText = daysUntilDue + (daysUntilDue == 1 ? " day out" : " days out");
            } else if (daysUntilDue < 0) {
                dueText = -daysUntilDue + (daysUntilDue == -1 ? " day late" : " days late");
            } else {
                dueText = "Due today";  // Case for current day
            }
        } else {
            // For months, handle both positive and negative cases
            if (monthsUntilDue >= 0) {
        ...
        // If more than 1 year difference, handle "years out" or "years late"
        if (yearsUntilDue >= 0) {
