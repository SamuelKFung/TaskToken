...
function displayMytaskCard(doc) { 
    ...
    // Calculate the total time difference in milliseconds between today and due date
    let timeDiff = due - today; 
    let daysUntilDue = Math.floor(timeDiff / (1000 * 60 * 60 * 24)); // Convert milliseconds to full days
    let yearsUntilDue = due.getFullYear() - today.getFullYear(); 
    let monthsUntilDue = due.getMonth() - today.getMonth() + (12 * yearsUntilDue);  // Total months difference
    ...
    let pillBadgeElement = name + "<span class=\"badge rounded-pill card-due fs-5 mx-4 mt-auto mb-auto " + pillBadgeColor + "\">" + (daysUntilDue >= 0 ? daysUntilDue : 0) + "</span>"; 
    ...
    if (Math.abs(yearsUntilDue) < 1) { 
        if (Math.abs(monthsUntilDue) < 1) { 
            ...
            dueText = (monthsUntilDue >= 0 ? monthsUntilDue : -monthsUntilDue) + (Math.abs(monthsUntilDue) == 1 ? " month out" : " months out"); 
        } 
    } else { 
        dueText = (yearsUntilDue >= 0 ? yearsUntilDue : -yearsUntilDue) + (Math.abs(yearsUntilDue) == 1 ? " year out" : " years out"); 
    } 
