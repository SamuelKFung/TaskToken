...
// Define global taskId variable to store the task to be deleted
let taskToDeleteId = null;

// Add Event Listener to the Delete button in the Task Card
document.addEventListener("DOMContentLoaded", function() {
    // For each delete button (inside a task card), add event listener
    document.querySelectorAll('.btn-danger').forEach(button => {
        button.addEventListener('click', function(event) {
            // Get the task ID (could be stored in the data attribute of the button)
            taskToDeleteId = event.target.dataset.taskId;
    ...
    // Confirm Deletion (Clicking 'Delete Task' button in the modal)
...
// Function to delete the task (from the UI and possibly from a database)
function deleteTask(taskId) {
    // Step 1: Remove the task from the DOM
    const taskCard = document.querySelector(`[data-task-id="${taskId}"]`);
    if (taskCard) {
        taskCard.remove();
    }

    // Step 2: If you're using Firebase or another backend, remove the task from there as well
    // For Firebase Firestore, you'd use something like:
    // firebase.firestore().collection("tasks").doc(taskId).delete()
    // Or use other data removal logic depending on where you're storing your data.

    // Close the delete modal after deleting the task
