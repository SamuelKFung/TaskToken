...
function displayMytaskCard(doc) { 
    ...
    if (Math.abs(yearsUntilDue) < 1) { 
        if (Math.abs(monthsUntilDue) < 1) { 
            ...
            dueText = (monthsUntilDue >= 1 ? monthsUntilDue : -monthsUntilDue) + (monthsUntilDue == 1 ? " month out" : " months out"); 
        } 
    } else { 
        dueText = (yearsUntilDue >= 1 ? yearsUntilDue : -yearsUntilDue) + (yearsUntilDue == 1 ? " year out" : " years out"); 
    } 
