...
function displayMytaskCard(doc) { 
   ...
   // Calculate the total difference in days between the current date and the due date
let daysUntilDue = Math.ceil((due - today) / (1000 * 60 * 60 * 24)); // Difference in days

// Convert days into months or years if necessary
let monthsUntilDue = due.getMonth() - today.getMonth() + (12 * (due.getFullYear() - today.getFullYear())); // Difference in months
let yearsUntilDue = due.getFullYear() - today.getFullYear(); // Difference in years

// Now handle the logic based on the days difference and show appropriate message
if (Math.abs(yearsUntilDue) < 1) { 
    if (Math.abs(monthsUntilDue) < 1) { 
        ...
        // If the due date is more than a month away, show months out
        dueText = (monthsUntilDue >= 0 ? monthsUntilDue : -monthsUntilDue) + 
                  (Math.abs(monthsUntilDue) == 1 ? " month out" : " months out"); 
    } 
} else { 
    // If the due date is more than a year away, show years out
    dueText = (yearsUntilDue >= 0 ? yearsUntilDue : -yearsUntilDue) + 
              (Math.abs(yearsUntilDue) == 1 ? " year out" : " years out"); 
}
