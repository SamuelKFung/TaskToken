...
function displayMytaskCard(doc) {
    ...
    // Color-coding logic based on due date
    let pillBadgeColor; 
    if ((daysUntilDue >= 3 && monthsUntilDue == 0 && yearsUntilDue == 0) || (monthsUntilDue > 0) || (yearsUntilDue > 0)) { 
        pillBadgeColor = "text-bg-success"; // Green for upcoming tasks
    } else if (daysUntilDue >= 0 && daysUntilDue < 3 && monthsUntilDue == 0 && yearsUntilDue == 0) { 
        pillBadgeColor = "text-bg-warning"; // Yellow for tasks that are due soon
    } else if (daysUntilDue < 0 || monthsUntilDue < 0 || yearsUntilDue < 0) { 
        pillBadgeColor = "text-bg-danger"; // Red for overdue tasks
    } else { 
        pillBadgeColor = "bg-primary"; // Default (should not show under correct logic)
    }

    // Additional logic to highlight overdue tasks today
    if (daysUntilDue == 0 && monthsUntilDue == 0 && yearsUntilDue == 0) {
        pillBadgeColor += " border border-danger border-5"; // Add border to highlight urgency
    }
...
function updateTask(taskId) {
    ...
    var updatedName = document.getElementById('title').value;
    var updatedCourse = document.getElementById('course').value;
    ...
    var updatedDueDate = document.getElementById('date').value;
    ...
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            ...
            taskRef.update({
                ...
                status: false // Or leave as is if you don't want to change this
            }).then(() => {
...
let editingTaskId = null;  // Variable to track if we're editing a task

// Function to show the task form with pre-filled data (for editing)
function showEditForm(taskId, taskData) {
    // Fill form fields with existing task data
    document.getElementById("task-name").value = taskData.name;
    document.getElementById("task-description").value = taskData.description;
    document.getElementById("task-duedate").value = taskData.duedate;
    document.getElementById("task-category").value = taskData.category;

    // Set the taskId we're editing
    editingTaskId = taskId;

    // Change the submit button text to "Edit Task"
    document.getElementById("task-submit-btn").innerText = "Edit Task";
}

// Function to clear the form and reset the editingTaskId
function clearForm() {
...
// Listen to the task form submit button
document.getElementById("task-submit-btn").addEventListener("click", function() {
    ...
    if (editingTaskId) {
        ...
        updateTask(editingTaskId, name, description, dueDate, category);
    } else {
        // If we're adding a new task, create a new one
        addTask(name, description, dueDate, category);
    }
    clearForm();  // Reset form after action
});
