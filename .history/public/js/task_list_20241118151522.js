...
// Global variable to store the task element to be deleted
let taskToDelete = null;

// Function to set the task to be deleted
function setTaskToDelete(taskId) {
    // Store the task element in the global variable based on the taskId
    taskToDelete = document.getElementById(taskId);
}

// Function to delete the task when the user confirms
function deleteTask() {
    if (taskToDelete) {
        // Remove the task element from the DOM
        taskToDelete.remove();
        
        // Optionally, send an AJAX request to delete the task from the server
        // Example: 
        // fetch('/delete-task', { method: 'POST', body: JSON.stringify({ taskId: taskToDelete.id }) });

        // Close the modal after the task is deleted
